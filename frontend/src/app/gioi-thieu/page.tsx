...
const steps = [
  ...
  { number: '02', title: 'Đối chiếu và xác minh', description: 'QR nối vùng trồng, nhật ký và dữ liệu đã được xác minh.', icon: CheckCircle2 },
  ...
  return (
    <PublicShell>
        ...
        <section className="mt-8 grid gap-5 lg:grid-cols-[0.9fr_1.1fr] lg:items-start sm:mt-10">
          <article className="rounded-[1.8rem] bg-[var(--brand-primary)] p-6 text-white shadow-[0_20px_45px_rgba(15,81,91,0.16)] sm:p-8"><p className="text-xs font-bold uppercase tracking-[0.18em] text-white/75">Ai cùng tham gia?</p><h2 className="mt-3 max-w-[20ch] text-3xl font-extrabold leading-tight sm:text-4xl lg:max-w-none lg:text-[2rem] xl:text-[2.25rem]">Một hệ sinh thái, nhiều vai trò<br className="hidden lg:block" /> rõ ràng</h2><p className="mt-4 text-sm leading-7 text-white/80">Mỗi chủ thể đóng góp một phần dữ liệu và nhận lại một cách trình bày phù hợp với nhu cầu của mình.</p><PublicSpeakButton className="mt-5" text="Ai cùng tham gia? Một hệ sinh thái, nhiều vai trò rõ ràng. Mỗi chủ thể đóng góp một phần dữ liệu và nhận lại một cách trình bày phù hợp với nhu cầu của mình." /></article>
        ...
        <section className="mt-8 rounded-[1.8rem] border border-[var(--border)] bg-[var(--surface-muted)] p-5 sm:mt-10 sm:p-8" aria-labelledby="principles-title"><p className="text-xs font-bold uppercase tracking-[0.18em] text-[var(--brand-primary)]">Nguyên tắc công khai</p><h2 id="principles-title" className="mt-3 text-[1.35rem] font-extrabold leading-tight tracking-[-0.025em] text-[var(--text-primary)] sm:text-3xl"><span className="whitespace-nowrap">Rõ dữ liệu, đúng nhận diện,</span>{' '}<span className="whitespace-nowrap">không phóng đại</span></h2><div className="mt-5 grid gap-3 sm:grid-cols-3"><p className="rounded-2xl bg-white px-4 py-4 text-sm font-semibold leading-6 text-[var(--text-secondary)]">Chỉ hiển thị hồ sơ đã được xác minh</p><p className="rounded-2xl bg-white px-4 py-4 text-sm font-semibold leading-6 text-[var(--text-secondary)]">Không dùng ảnh ngẫu nhiên</p><p className="rounded-2xl bg-white px-4 py-4 text-sm font-semibold leading-6 text-[var(--text-secondary)]">Mở QR để xem đúng hồ sơ công khai</p></div></section>
      </PublicPageMain>
    </PublicShell>
  );
}
