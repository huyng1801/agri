...
export function PublicOutcomeShowcase({
  ...
  return (
    <div className={cn('mt-8', className)}>
      <div className="relative mx-auto max-w-[72rem] px-12 sm:px-16 lg:px-10">
        <button
          type="button"
          aria-label="Lợi ích trước"
          className="absolute left-0 top-[8.8rem] grid h-12 w-12 place-items-center rounded-[1rem] bg-[rgba(228,228,228,0.84)] text-slate-500 shadow-sm backdrop-blur transition hover:bg-[rgba(214,214,214,0.94)] hover:text-[#1f2233] sm:top-[10.8rem] sm:h-14 sm:w-14 lg:top-[3.5rem]"
        ...
        <button
          ...
          className="absolute right-0 top-[8.8rem] grid h-12 w-12 place-items-center rounded-[1rem] bg-[rgba(228,228,228,0.84)] text-slate-500 shadow-sm backdrop-blur transition hover:bg-[rgba(214,214,214,0.94)] hover:text-[#1f2233] sm:top-[10.8rem] sm:h-14 sm:w-14 lg:top-[3.5rem]"
          onClick={() => setIndex((value) => value + 1)}
        >
        ...
        <div className="relative text-center">
          ...
          <div className="relative hidden lg:block">
            <div
              aria-hidden="true"
              className="absolute inset-x-[10%] top-8 h-20 rounded-full bg-[radial-gradient(circle,rgba(31,155,75,0.08),transparent_72%)] blur-3xl"
            />
            <div className="relative mx-auto grid max-w-6xl grid-cols-6 gap-5 xl:gap-8">
                  ...
                  >
                    <span
                      className={cn(
                        ...
                        activeItem ? 'bg-[#f2fbf2] text-[#2b8a3e]' : 'bg-transparent text-[#5a9a66] hover:text-[#2b8a3e]'
                      )}
                    >
                      <ItemIcon size={68} strokeWidth={1.45} aria-hidden="true" />
                    </span>
                    <span
                      className={cn(
                        'mt-3 text-[0.72rem] font-semibold uppercase tracking-[0.18em] transition',
                        activeItem ? 'text-[#1f2233]' : 'text-slate-400 group-hover:text-slate-600'
                      )}
                    >
                      {item.title}
                    </span>
          ...
          <div className="mt-7 flex flex-wrap items-center justify-center gap-2 lg:hidden">
            {items.map((item, itemIndex) => (
              <button
                key={`${item.title}-${item.value}`}
                type="button"
