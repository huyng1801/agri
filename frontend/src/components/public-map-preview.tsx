...
export function PublicMapPreview({ address, location, mapSearchUrl, className, frameClassName, compact = false }: PublicMapPreviewProps) {
  ...
  return (
    <div className={cn('overflow-hidden rounded-[1.7rem] border border-white/18 bg-[#d9eadf]', className)}>
      <div className={cn('relative isolate w-full overflow-hidden', compact ? 'aspect-[1.1/1]' : 'aspect-[1.28/1] min-h-[240px] sm:min-h-[280px]')}>
        ...
        <div className={cn('absolute inset-0 grid grid-cols-3 grid-rows-3 opacity-95', frameClassName)}>
          {tiles.map((tile) => (
            <img
              key={`${tile.x}-${tile.y}`}
